...
public class OctaveParser implements PsiParser {
  ...
  public static final Logger LOG_ = Logger.getInstance("ru.compscicenter.jetbrains.octave.parser.OctaveParser");
  ...
  protected boolean parse_root_(final IElementType root_, final PsiBuilder builder_, final int level_) {
    return octaveFile(builder_, level_ + 1);
  }
  ...
  /* ********************************************************** */
  // item_*
  static boolean octaveFile(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "octaveFile")) return false;
    int pos_ = current_position_(builder_);
    while (true) {
      if (!item_(builder_, level_ + 1)) break;
      if (!empty_element_parsed_guard_(builder_, "octaveFile", pos_)) break;
